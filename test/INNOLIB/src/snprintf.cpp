/* @(#)root/clib:$Name:  $:$Id: snprintf.cpp,v 1.3 2009/03/24 07:33:28 kskim Exp $ */
            ...
            *str++ = '%';
            ...
            if (left <= 0) {
               *str = '\0';
               ...
               precision_specified = 0;	// HJSON; fixed blabla...
...
int _mysnprintf(char *str, size_t size, const char *format, ...)
{
   int ret;
   va_list ap;
   va_start(ap, format);
   ret = myvsnprintf(str, size, format, ap);
   va_end(ap);

   return ret;
}

#ifdef _SNPRINTF_TEST
#include <stdio.h>

char* suffixes[10] = { " ", "K" };

/* set of small tests for _mysnprintf() */
int main()
{
   char holder[100];
   int i;
   ...
   printf("ihuman n : %d, unit : %s, size : %d\n", n, suffixes[n], len); 
   _mysnprintf(buf,len,"%.1f%s",val,suffixes[n]);
   printf("HJSON: ihuman result : %s\n", buf); 
   _mysnprintf(buf,len,"%d%s",123,suffixes[n]);
   printf("ihuman result : %s\n", buf); 
   _mysnprintf(holder,100,"a = %I64d",a);
   printf("%s\n",holder);

   return 0;

/*
   printf("Suite of test for _mysnprintf:\n");
   printf("a_format\n");
   printf("printf() format\n");
   printf("_mysnprintf() format\n\n");
*/

   /* Checking the field widths */
   printf("/%%d/, 336\n");
   _mysnprintf(holder, sizeof holder, "/%d/\n", 336);
   printf("/%d/\n", 336);
   printf("%s\n", holder);

   printf("/%%2d/, 336\n");
   _mysnprintf(holder, sizeof holder, "/%2d/\n", 336);
   printf("/%2d/\n", 336);
   printf("%s\n", holder);

   printf("/%%10d/, 336\n");
   _mysnprintf(holder, sizeof holder, "/%10d/\n", 336);
   printf("/%10d/\n", 336);
   printf("%s\n", holder);
   ...
   /* floating points */
   printf("/%%f/, 1234.56\n");
   _mysnprintf(holder, sizeof holder, "/%f/\n", 1234.56);
   printf("/%f/\n", 1234.56);
   printf("%s\n", holder);

   printf("/%%e/, 1234.56\n");
   _mysnprintf(holder, sizeof holder, "/%e/\n", 1234.56);
   printf("/%e/\n", 1234.56);
   printf("%s\n", holder);

   printf("/%%4.2f/, 1234.56\n");
   _mysnprintf(holder, sizeof holder, "/%4.2f/\n", 1234.56);
   printf("/%4.2f/\n", 1234.56);
   printf("%s\n", holder);

   printf("/%%3.1f/, 1234.56\n");
   _mysnprintf(holder, sizeof holder, "/%3.1f/\n", 1234.56);
   printf("/%3.1f/\n", 1234.56);
   printf("%s\n", holder);

   printf("/%%10.3f/, 1234.56\n");
   _mysnprintf(holder, sizeof holder, "/%10.3f/\n", 1234.56);
   printf("/%10.3f/\n", 1234.56);
   printf("%s\n", holder);

   printf("/%%10.3e/, 1234.56\n");
   _mysnprintf(holder, sizeof holder, "/%10.3e/\n", 1234.56);
   printf("/%10.3e/\n", 1234.56);
   printf("%s\n", holder);
   ...
   _mysnprintf(holder, sizeof holder, "/%010.2f/\n", 1234.56);
   printf("/%010.2f/\n", 1234.56);
   printf("%s\n", holder);
...
#define BLURB "Outstanding acting !"
   /* strings precisions */
   printf("/%%2s/, \"%s\"\n", BLURB);
   _mysnprintf(holder, sizeof holder, "/%2s/\n", BLURB);
   printf("/%2s/\n", BLURB);
   printf("%s\n", holder);

   printf("/%%22s/ %s\n", BLURB);
   _mysnprintf(holder, sizeof holder, "/%22s/\n", BLURB);
   printf("/%22s/\n", BLURB);
   printf("%s\n", holder);

   printf("/%%22.5s/ %s\n", BLURB);
   _mysnprintf(holder, sizeof holder, "/%22.5s/\n", BLURB);
   printf("/%22.5s/\n", BLURB);
   printf("%s\n", holder);
   ...
   _mysnprintf(holder, sizeof holder, "%x %X %#x\n", 31, 31, 31);
   printf("%x %X %#x\n", 31, 31, 31);
   printf("%s\n", holder);

   printf("**%%d**%% d**%% d**, 42, 42, -42\n");
   _mysnprintf(holder, sizeof holder, "**%d**% d**% d**\n", 42, 42, -42);
   printf("**%d**% d**% d**\n", 42, 42, -42);
   printf("%s\n", holder);

   /* other flags */
   printf("/%%g/, 31.4\n");
   _mysnprintf(holder, sizeof holder, "/%g/\n", 31.4);
   printf("/%g/\n", 31.4);
   printf("%s\n", holder);

   printf("/%%.6g/, 31.4\n");
   _mysnprintf(holder, sizeof holder, "/%.6g/\n", 31.4);
   printf("/%.6g/\n", 31.4);
   printf("%s\n", holder);

   printf("/%%.1G/, 31.4\n");
   _mysnprintf(holder, sizeof holder, "/%.1G/\n", 31.4);
   printf("/%.1G/\n", 31.4);
   printf("%s\n", holder);
   ...
   printf("abc%n", &i);
   printf("%d\n", i);
   _mysnprintf(holder, sizeof holder, "abc%n", &i);
   printf("%s", holder);
   printf("%d\n\n", i);

   printf("%%*.*s --> 10.10\n");
   _mysnprintf(holder, sizeof holder, "%*.*s\n", 10, 10, BLURB);
   printf("%*.*s\n", 10, 10, BLURB);
   printf("%s\n", holder);

   printf("%%%%%%%%\n");
   _mysnprintf(holder, sizeof holder, "%%%%\n");
   printf("%%%%\n");
   printf("%s\n", holder);
...
#define BIG "Hello this is a too big string for the buffer"
   ...
   printf("<%%s>, %s\n", BIG);
   i = _mysnprintf(holder, 10, "%s", BIG);
   printf("<%s>\n", BIG);
   printf("<%s> i = %d\n", holder, i);
   i = _mysnprintf(holder, sizeof holder, "%s", BIG);
   printf("<%s> i = %d\n", holder, i);
